...
extern "C" __declspec(dllexport) void SetAppIcon(HWND hwnd, const wchar_t* icon_path) {
  if (hwnd == nullptr || icon_path == nullptr) {
    return;
  }

  // Load the icon from the file path.
  HICON hIcon = (HICON)LoadImageW(NULL, icon_path, IMAGE_ICON, 0, 0,
                                 LR_LOADFROMFILE | LR_DEFAULTSIZE);

  if (hIcon) {
    // Set the big icon (Alt+Tab) and small icon (Taskbar and Title bar).
